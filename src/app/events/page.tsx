...
async function getEvents() {
    try {
        const [upRes, pastRes] = await Promise.all([
            fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://paleturquoise-caribou-901347.hostingersite.com"}/api/v1/events/upcoming`, { cache: "no-store" }),
            fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://paleturquoise-caribou-901347.hostingersite.com"}/api/v1/events/past`, { cache: "no-store" })
        ]);
